...
|--------|------|-------------|--------------|----------|
| [tokenization-pepper](#tokenization-pepper) | HMAC pepper | Secret Manager (PII Vault) | dev, prod | FastAPI, ingest job, report job, vault service |
| [pii-tokenization-key](#pii-tokenization-key) | Encryption key | Secret Manager (PII Vault) | dev, prod | FastAPI, ingest job, report job, vault service |
...
| [Dev API tokens](#dev-api-tokens) | Hardcoded tokens | Source code | local, dev | FastAPI auth |
...
| **Injected into** | FastAPI service, ingest job, report job, vault Cloud Run service |
...
| | |
|---|---|
| **Purpose** | API authentication token for service-to-service calls. The console (Next.js) and intake worker use this token to authenticate against the FastAPI backend via the `X-API-KEY` header. |
...
> then rebuild and redeploy the FastAPI image.

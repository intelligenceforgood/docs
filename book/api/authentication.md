...
API access is tightly controlled because the platform processes highly sensitive PII. Current authentication and future enhancements are summarized below.

## Current State

- **Provider:** Google Identity Platform (OAuth 2.0) with email-domain allowlists.
- **Flow:** Clients obtain an ID token via Google Sign-In and exchange it for a session token managed by FastAPI.
